-- 084_admin_purge_clinical_data.sql
--
-- One-shot demo / pre-go-live cleanup: remove clinician-entered operational
-- data (visits, notes, billing rows, inpatient, ANC, etc.) strictly BEFORE a
-- cutoff date. Does NOT touch:
--   medical_corpus, medical_documents, lab_test_catalog, medication_catalog,
--   pharmacy_stock_*, lab_stock_*, clinic_lab_capabilities,
--   clinic_pharmacy_formulary, clinic_billing_rates, staff, clinics, HMIS codes.
--
-- Call from SQL editor (as superadmin JWT) or service role:
--   SELECT rpc_admin_purge_clinical_data_before('2026-06-05'::date, NULL, TRUE, TRUE);
-- Dry run first (default). Pass p_dry_run := FALSE to execute.
...
BEGIN
  ...
  -- RESTRICT FKs (063): payments and provider_notes block visit/patient deletes.
  ...
  -- provider_notes.patient_id is RESTRICT; remove before patients.
  ...
  IF p_delete_orphan_patients THEN
    ...
    -- pregnancies.patient_id RESTRICT; anc_contacts cascade from pregnancy.
    DELETE FROM pregnancies pg
    WHERE pg.patient_id IN (SELECT id FROM _purge_patients);

    -- admissions.patient_id RESTRICT; inpatient children cascade from admission.
